...
const initClientsPremium = (clients) => {
  ...
  tl.timeScale(isMobile ? 0.9 : 1.12)

  const reset = () => {
    if (vLine) gsap.set(vLine, { scaleY: 0, transformOrigin: '50% 0%' })
    if (hLine) gsap.set(hLine, { scaleX: 0, transformOrigin: '0% 50%' })
    
    if (titleChars.length) gsap.set(titleChars, { 
      opacity: 0, 
      yPercent: isMobile ? 30 : 130, 
      transformOrigin: '50% 100%' 
    })

    if (count) gsap.set(count, { opacity: 0, y: isMobile ? 5 : 10 })
    if (countMask) gsap.set(countMask, { scaleX: 1, transformOrigin: '0% 50%' })
    if (count) count.textContent = formatCountText(countOriginal, 0)

    if (logos.length) {
      gsap.set(logos, { opacity: 0, y: isMobile ? 15 : 20 })
    }

    if (seeMore) gsap.set(seeMore, { opacity: 0, y: isMobile ? 8 : 12 })
    if (seeMoreText) gsap.set(seeMoreText, { opacity: 0, y: isMobile ? 5 : 10 })
    if (seeMoreArrow) gsap.set(seeMoreArrow, { opacity: 0, x: isMobile ? -8 : -10, transformOrigin: '50% 50%' })

    tl.pause(0)
  }

  const tLines = 0
  const tTitle = isMobile ? 0.1 : 0.22
  const tCount = isMobile ? 0.35 : 0.55
  const tLogos = isMobile ? 0.45 : 0.78
  const tMore = isMobile ? 0.9 : 1.35

  if (vLine) tl.to(vLine, { scaleY: 1, duration: isMobile ? 0.6 : 0.85, ease: 'power2.out' }, tLines)
  if (hLine) tl.to(hLine, { scaleX: 1, duration: isMobile ? 0.6 : 0.85, ease: 'power2.out' }, tLines + 0.03)
  ...
  if (titleChars.length) {
    tl.to(titleChars, {
      ...
      ease: 'power4.out',
      stagger: { each: isMobile ? 0.015 : 0.022, from: 'start' }
    }, tTitle)
  }

  if (countMask) tl.to(countMask, { scaleX: 0, duration: isMobile ? 0.4 : 0.55, ease: 'power3.inOut' }, tCount)
  if (count) tl.to(count, { 
    opacity: 1, 
    y: 0, 
  ...
  if (seeMoreText) tl.to(seeMoreText, { 
    opacity: 1, 
    y: 0, 
  ...
  if (seeMoreArrow) tl.to(seeMoreArrow, { 
    opacity: 1, 
    x: 0, 
    duration: isMobile ? 0.5 : 0.7, 
    ease: 'power3.out' 
